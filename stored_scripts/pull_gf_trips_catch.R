...
here::i_am("stored_scripts/pull_gf_trips_catch.R")
...
latest_file <- folder_files %>%
  ...
  filter(str_detect(name, "^trip_catch_gf\\d{4}-\\d{2}-\\d{2}\\.Rds$")) %>%
...
if (nrow(latest_file) == 0) {
  stop("No files matching 'trip_catch_gf{YYYY-MM-DD}.Rds' were found in the folder.")
}
...
trip_catch_gf <- read_rds(temp_path)
...
write_rds(
  trip_catch_gf,
  file = here("data", "main", glue("trip_catch_gf{date_file}.Rds"))
)

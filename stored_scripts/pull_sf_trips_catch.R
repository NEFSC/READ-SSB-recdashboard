## This script reads in the MRIP historical directed trips and catch Rds for SFSBSB from Google Drive
...
here::i_am("stored_scripts/pull_sf_trips_catch.R")
...
folder_files <- drive_ls(
  ...
  q = "name contains 'trip_catch_sfsbsb'"
)
...
latest_file <- folder_files %>%
  ...
  filter(str_detect(name, "^trip_catch_sfsbsb\\d{4}-\\d{2}-\\d{2}\\.Rds$")) %>%
...
if (nrow(latest_file) == 0) {
  stop("No files matching 'trip_catch_sfsbsb{YYYY-MM-DD}.Rds' were found in the folder.")
}
...
trip_catch_sfsbsb <- read_rds(temp_path)
...
date_file <- as.character(latest_file$parsed_date)

write_rds(
  trip_catch_sfsbsb,
  file = here("data", "main", glue("trip_catch_sfsbsb{date_file}.Rds"))
)
